...
class NestedBoxDataProcessor(BaseDataProcessor):
    ...
    def _fallback_keyword_extraction_for_box_label(self, excel_file_path: str) -> Dict[str, Any]:
        """回退到关键字提取方式"""
        return self.extract_fallback_data(excel_file_path, self.get_fallback_keyword_config())
    ...
    def calculate_overweight_box_distribution(self, boxes_per_set: int, boxes_per_large_box: int, box_in_set: int) -> int:
        """计算超重模式下每箱的盒子分配"""
        return self.calculate_overweight_distribution(boxes_per_set, boxes_per_large_box, box_in_set)
    ...
    def generate_linear_serial_range(self, base_number: str, container_num: int,
        ...
        if container_type == "小箱":
            return self.generate_small_box_serial_range(base_number, container_num, items_per_container, total_items)
        else:
            return self.generate_large_box_serial_range(base_number, container_num, 1, items_per_container, total_items)
    ...
    def generate_grouped_serial_range(self, base_number: str, container_num: int,
        ...
        if container_type == "小箱":
            ...
            return self.generate_large_box_serial_range(base_number, container_num, group_size, items_per_container, total_items)
