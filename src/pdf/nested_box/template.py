...
class NestedBoxTemplate(PDFBaseUtils):
    ...
    def _create_normal_pdfs(
        ...
        if has_small_box:
            ...
            # 无小箱模式：直接从盒到箱
            math.ceil(total_boxes / boxes_per_small_box)
        ...
        large_box_filename = (
            f"{customer_code}_{chinese_name}_{english_name}_套盒箱标_{timestamp}.pdf"
        )
    ...
    def _create_overweight_pdfs(
        ...
        total_sets * boxes_per_large_box
    ...
    def _create_nested_box_label(
        ...
        # 套盒模板参数分析
        pieces_per_box = int(params["张/盒"])
        boxes_per_ending_unit = int(
            params["盒/套"]
        )  # 在套盒模板中，这个参数用于结束号的范围计算
        ...
        for box_num in range(1, total_boxes + 1):
            ...
            # 渲染外观
            if style == "外观一":
                nested_box_renderer.render_nested_appearance_one(
                    c,
                    width,
                    theme_text,
                    current_number,
                ...
                nested_box_renderer.render_nested_appearance_two(
                    c,
                    width,
                    theme_text,
                    current_number,
    ...
    def _create_nested_small_box_label(
        ...
        print(
            f"✅ 套盒套标使用统一数据: 主题='{theme_text}', 开始号='{base_number}', 客户编码='{remark_text}', 序列号字体大小={serial_font_size}"
        )
        ...
        pieces_per_box * boxes_per_small_box
        ...
        if template_type == "有纸卡备注":
            nested_box_renderer.render_empty_small_box_label(
                c,
                width,
                height,
                chinese_name,
        ...
        for small_box_num in range(1, total_small_boxes + 1):
            ...
            if template_type == "有纸卡备注":
                nested_box_renderer.draw_nested_small_box_table(
                    c,
                    width,
                    height,
                    theme_text,
                ...
                nested_box_renderer.draw_nested_small_box_table_no_paper_card(
                    c,
                    width,
                    height,
                    theme_text,
    ...
    def _create_nested_large_box_label(
        self,
        data: Dict[str, Any],
        params: Dict[str, Any],
        output_path: str,
        ...
        serial_font_size = int(params.get("序列号字体大小", 10))
        print(
            f"✅ 套盒箱标使用统一数据: 主题='{theme_text}', 开始号='{base_number}', 客户编码='{remark_text}', 序列号字体大小={serial_font_size}"
        )
        ...
        print(
            f"✅ 套盒箱标参数: 张/盒={pieces_per_box}, 盒/套={boxes_per_small_box}, 套/箱={small_boxes_per_large_box}"
        )
        ...
        print(
            f"✅ 计算结果: 每套{pieces_per_small_box}PCS, 每箱{pieces_per_large_box}PCS"
        )
        ...
        if template_type == "有纸卡备注":
            nested_box_renderer.render_empty_large_box_label(
                c,
                width,
                height,
                chinese_name,
        ...
        for large_box_num in range(1, total_large_boxes + 1):
            ...
            if template_type == "有纸卡备注":
                nested_box_renderer.draw_nested_large_box_table(
                    c,
                    width,
                    height,
                    theme_text,
                ...
                nested_box_renderer.draw_nested_large_box_table_no_paper_card(
                    c,
                    width,
                    height,
                    theme_text,
    ...
    def _create_overweight_large_box_label(
        ...
        serial_font_size = int(params.get("序列号字体大小", 10))
        print(
            f"✅ 超重模式箱标使用统一数据: 主题='{theme_text}', 开始号='{base_number}', 客户编码='{remark_text}', 序列号字体大小={serial_font_size}"
        )
        ...
        print(
            f"✅ 超重模式箱标参数: 张/盒={pieces_per_box}, 盒/套={boxes_per_set}, 一套拆{boxes_per_large_box}箱"
        )
        ...
        print(
            f"✅ 超重模式计算结果: 总盒={total_boxes}, 总套={total_sets}, 总箱={total_large_boxes}"
        )
        ...
        for large_box_num in range(1, total_large_boxes + 1):
            ...
            boxes_in_each_box = (
                nested_box_data_processor.calculate_overweight_box_distribution(
