...
class RegularDataProcessor(BaseDataProcessor):
    ...
    def calculate_multi_level_quantities(self, total_pieces: int, pieces_per_box: int, 
        ...
        quantities = super().calculate_multi_level_quantities(
            total_pieces, pieces_per_box, boxes_per_small_box, small_boxes_per_large_box
        )
        
        # 添加常规模板特有的余数信息
        quantities.update({
            'remaining_pieces_in_last_box': total_pieces % pieces_per_box,
            'remaining_boxes_in_last_small_box': quantities['total_boxes'] % boxes_per_small_box,
            'remaining_small_boxes_in_last_large_box': quantities['total_small_boxes'] % small_boxes_per_large_box
        })
    ...
    def generate_linear_serial_range(self, base_number: str, container_num: int,
                                   items_per_container: int, container_type: str = "小箱", 
        ...
        print(f"📝 常规{container_type}标 #{container_num}: 包含盒{start_item}-{end_item}, 序列号范围={serial_range}")
        return serial_range
    ...
    def generate_regular_small_box_serial_range(self, base_number: str, small_box_num: int, 
        ...
        """向后兼容包装器"""
        return self.generate_linear_serial_range(base_number, small_box_num, boxes_per_small_box, "小箱", total_boxes)
    ...
    def generate_regular_large_box_serial_range(self, base_number: str, large_box_num: int,
        ...
        return self.generate_linear_serial_range(
            base_number, large_box_num, end_box - start_box + 1, "大箱", total_boxes
        )
    ...
    def generate_grouped_serial_number(self, base_number: str, box_num: int, group_size: int) -> str:
        """
    ...
    def generate_grouped_serial_range(self, base_number: str, container_num: int,
                                    items_per_container: int, group_size: int,
                                    container_type: str = "小箱", total_items: int = None) -> str:
        """
        生成分组序列号范围 - 常规模板不使用分组，委托给线性算法
        """
        # 常规模板使用简单的线性递增，忽略group_size参数
        return self.generate_linear_serial_range(base_number, container_num, items_per_container, container_type, total_items)
