"""
常规模板UI对话框
基于BaseUIDialog重构，专门处理常规模板的参数设置界面
"""
...
class RegularUIDialog(BaseUIDialog):
    """常规模板UI对话框处理类"""
    ...
    def show_parameters_dialog(self):
        """显示常规模板参数设置对话框"""
        # 创建对话框
        dialog = self.create_base_dialog("常规模板 - 包装参数设置", 580, 480)
        if not dialog:
            return
        ...
        # 创建标题
        self.create_title_label(main_frame, "常规模板参数设置")
    ...
    def confirm_parameters(self):
        ...
        try:
            ...
            self.main_app.packaging_params = {
                ...
                "选择外观": self.get_var_value("appearance"),
    ...
    def get_template_specific_inputs(self) -> List[Dict]:
        """获取常规模板特定的输入配置"""
        return [
            {'label': '张/盒:', 'var_name': 'pieces_per_box', 'default_value': '', 'width': 20},
            {'label': '盒/小箱:', 'var_name': 'boxes_per_small_box', 'default_value': '', 'width': 20},
            {'label': '小箱/大箱:', 'var_name': 'small_boxes_per_large_box', 'default_value': '', 'width': 20},
            {'label': '中文名称:', 'var_name': 'chinese_name', 'default_value': '', 'width': 20},
            {'label': '序列号字体大小:', 'var_name': 'serial_font_size', 'default_value': '10', 'width': 20}
        ]
