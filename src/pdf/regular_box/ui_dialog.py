...
class RegularUIDialog(BaseUIDialog):
    ...
    def show_parameters_dialog(self):
        ...
        for widget in params_frame.winfo_children():
            ...
            elif isinstance(widget, ttk.Entry) and widget.cget("textvariable") == str(
                self.get_var("small_boxes_per_large_box")
            ):
    ...
    def on_small_box_choice_changed(self):
        """处理小箱选择变化"""
        if (
            self.second_param_label
            and self.third_param_label
            and self.third_param_entry
        ):
        ...
        try:
            ...
            if has_small_box:
                small_boxes_per_large_box = self.validate_integer_input(
                    "small_boxes_per_large_box", "小箱/大箱", 1
                )
            else:
