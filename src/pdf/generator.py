...
class PDFGenerator:
    ...
    def create_multi_level_pdfs(
        ...
        创建常规模板的多级标签PDF
        """
        return self.regular_template.create_multi_level_pdfs(
            data, params, output_dir, excel_file_path
        )

    def create_split_box_multi_level_pdfs(
        self,
        data: Dict[str, Any],
        params: Dict[str, Any],
        output_dir: str,
        ...
        """
        return self.split_box_template.create_multi_level_pdfs(
            data, params, output_dir, excel_file_path
        )

    def create_nested_box_multi_level_pdfs(
        self,
        data: Dict[str, Any],
        params: Dict[str, Any],
        output_dir: str,
        ...
        """
        return self.nested_box_template.create_multi_level_pdfs(
            data, params, output_dir, excel_file_path
        )
    ...
    def set_page_size(self, size: str):
        ...
        (
            self.regular_template.set_page_size(size)
            if hasattr(self.regular_template, "set_page_size")
            else None
        )
        (
            self.split_box_template.set_page_size(size)
            if hasattr(self.split_box_template, "set_page_size")
            else None
        )
        (
            self.nested_box_template.set_page_size(size)
            if hasattr(self.nested_box_template, "set_page_size")
            else None
        )
