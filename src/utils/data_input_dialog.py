...
class DataInputDialog:
    ...
    def _create_data_table(self, parent, missing_fields):
        ...
        ttk.Label(header_frame, text="数据字段", font=("Arial", 11, "bold"), width=12).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(header_frame, text="当前值", font=("Arial", 11, "bold"), width=30).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(header_frame, text="状态", font=("Arial", 11, "bold"), width=8).pack(side=tk.LEFT)
    ...
    def _create_field_row(self, parent, field_name, is_missing):
        ...
        if is_missing:
            ...
            entry = ttk.Entry(input_frame, textvariable=self.input_vars[field_name], width=25, font=("Arial", 10))
            entry.pack(side=tk.LEFT)
            
            # 格式提示
            hint_text = self.required_fields[field_name]["format_hint"]
            hint_label = ttk.Label(input_frame, text=f"  格式：{hint_text}", font=("Arial", 9), foreground="gray")
            hint_label.pack(side=tk.LEFT)
            
            # 状态图标
            status_label = ttk.Label(row_frame, text="❌ 缺失", foreground="red", width=8)
            ...
            # 已有数据 - 显示当前值
            value_label = ttk.Label(value_frame, text=current_value, font=("Arial", 10, "bold"), width=30, anchor=tk.W)
            value_label.pack(anchor=tk.W)
            
            # 状态图标
            status_label = ttk.Label(row_frame, text="✅ 已有", foreground="green", width=8)
            status_label.pack(side=tk.LEFT)
    ...
    def _validate_client_code(self, value):
        """验证客户编码"""
        if len(value) < 2:
            messagebox.showerror("格式错误", "客户编码长度至少2个字符\n\n正确格式：14KH0149、ABC123")
            return False
        return True
    ...
    def _validate_start_number(self, value):
        """验证开始号"""
        if len(value) < 3:
            messagebox.showerror("格式错误", "开始号格式不正确\n\n正确格式：DSK00001、JAW01001-01")
            return False
        return True
    ...
    def _validate_total_count(self, value):
        ...
        try:
            count = int(value)
            if count <= 0:
                messagebox.showerror("格式错误", "总张数必须为正整数\n\n正确格式：57000、1000")
                return False
            return True
        except ValueError:
