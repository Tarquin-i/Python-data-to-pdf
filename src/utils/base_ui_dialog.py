...
class BaseUIDialog(ABC):
    ...
    def create_radio_group(self, parent, title: str, options: List[Tuple[str, str]], 
                          var_name: str, default_value: str = None, command=None):
        """
        创建单选按钮组
    ...
    def validate_integer_input(self, var_name: str, field_name: str, min_value=1, max_value=None) -> int:
        """
        验证整数输入
        ...
        try:
            ...
            raise ValueError(f"'{field_name}'必须为有效整数\\n\\n正确格式示例：{min_value}")
    ...
    def validate_pieces_per_box(self) -> int:
        ...
        pieces_per_box = self.validate_integer_input('pieces_per_box', '张/盒', 1, total_pieces)
        
        if pieces_per_box > total_pieces:
            raise ValueError(f"'张/盒'不能超过总张数\\n\\n当前设置：{pieces_per_box} 张/盒\\n总张数：{total_pieces} 张\\n\\n请输入不超过{total_pieces}的值")
    ...
    def validate_font_size(self, var_name: str = 'serial_font_size', min_size=6, max_size=14) -> int:
        """验证字体大小"""
        try:
            font_size = self.validate_integer_input(var_name, '序列号字体大小', min_size, max_size)
            return font_size
        except ValueError as e:
            if "大于等于" in str(e) or "不能超过" in str(e):
                raise ValueError(f"序列号字体大小必须在{min_size}-{max_size}之间\\n\\n当前值：{self.input_vars[var_name].get()}")
            else:
                raise ValueError(f"请输入有效的序列号字体大小\\n\\n正确格式示例：10")
    ...
    def get_var_value(self, var_name: str):
        """获取变量值"""
        return self.input_vars.get(var_name).get() if var_name in self.input_vars else None
