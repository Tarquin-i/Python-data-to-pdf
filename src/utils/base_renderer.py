...
class BaseRenderer(ABC):
    ...
    def render_centered_title(
        ...
        """渲染居中的标题（两行版本：标题+序列号）"""
        return self.render_two_line_label(
            c,
            width,
            top_text,
            serial_number,
    ...
    def render_centered_chinese_title(self, c, width, height, title_text, font_size=14):
        ...
        if text_width > max_width:
            ...
            title_lines = text_processor.wrap_text_to_fit(
                c, clean_title, max_width, current_font_name, font_size
            )
    ...
    def render_two_line_label(
        ...
        if text_width > max_width:
            # 需要换行
            title_lines = text_processor.wrap_text_to_fit(
                c, clean_title, max_width, current_font_name, title_font_size
            )
    ...
    def draw_base_table_structure(
        ...
        actual_heights = [
            table_height * row["height_ratio"] / total_ratio for row in rows_config
        ]
    ...
    def draw_table_cell_text(
        ...
        if max_width is None:
            ...
            text_lines = text_processor.wrap_text_to_fit(
                c, clean_text, max_width, current_font_name, font_size
            )
    ...
    def draw_standard_box_table(
        ...
        quantity_row_idx = 2
        quantity_split_y = (
            table_info["row_positions"][quantity_row_idx]
            + table_info["row_heights"][quantity_row_idx] / 2
        )
        c.line(
            table_info["table_x"] + table_info["label_col_width"],
            quantity_split_y,
            table_info["table_x"] + table_info["table_width"],
            quantity_split_y,
        )
        ...
        if has_paper_card:
            ...
            # Item行 (第5行，从上往下)
            item_y = (
                table_info["row_positions"][4]
                + table_info["row_heights"][4] / 2
                - text_offset
            )
        ...
        # Quantity行 (第3行，双倍高度)
        quantity_label_y = (
            table_info["row_positions"][2]
            + table_info["row_heights"][2] / 2
            - text_offset
        )
        ...
        # Carton No行
        carton_y = (
            table_info["row_positions"][1]
            + table_info["row_heights"][1] / 2
            - text_offset
        )
        ...
        # Remark行
        remark_y = (
            table_info["row_positions"][0]
            + table_info["row_heights"][0] / 2
            - text_offset
        )
    ...
    def render_blank_box_first_page(
        self, c, width, height, chinese_name: str, font_size=22
    ):
        """
        ...
        """
        return self.render_centered_chinese_title(
            c, width, height, chinese_name, font_size
        )
    ...
    def render_empty_box_label_with_paper_card(
        self, c, width, height, chinese_name, remark_text
    ):
        """渲染有纸卡备注的空箱标签 - 调用主实现（2025重新设计版本）"""
        return self.render_empty_box_label(
            c, width, height, chinese_name, remark_text, has_paper_card=True
        )

    def render_empty_box_label_without_paper_card(
        self, c, width, height, chinese_name, remark_text
    ):
