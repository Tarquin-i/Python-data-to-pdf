...
class TextProcessor:
    ...
    def clean_text_for_font(self, text) -> str:
        ...
        # 移除不可打印的字符，保留空格和换行
        cleaned = ''.join(char for char in str(text) if char.isprintable() or char.isspace())
        return cleaned.strip()
    
    def wrap_text_to_fit(self, canvas_obj: canvas.Canvas, text: str, max_width: float, 
    ...
    def format_serial_number(self, prefix: str, main_number: int, suffix: int, 
                           main_digits: int = 5, suffix_digits: int = 2) -> str:
        """
        格式化序列号
