...
class BaseDataProcessor(ABC):
    ...
    def extract_fallback_data(self, excel_file_path: str, keyword_config: Dict[str, Dict]) -> Dict[str, Any]:
        """
        回退到关键字提取方式
        当标准提取失败时使用的备用方法
        """
    ...
    def generate_serial_number(self, base_number: str, index: int, increment_type='suffix') -> str:
        """
        生成序列号
        ...
        if increment_type == 'suffix' and serial_info['has_suffix']:
            ...
            if serial_info['has_suffix']:
                ...
                current_number = f"{serial_info['prefix']}{new_main:0{serial_info['main_digits']}d}"
        else:
            # 混合递增或其他情况，子类可以重写实现
    ...
    def generate_serial_range(self, base_number: str, start_index: int, count: int) -> str:
        """
        生成序列号范围字符串
    ...
    def _generate_custom_serial_number(self, serial_info: Dict[str, Any], index: int) -> str:
        """
        自定义序列号生成逻辑，子类可以重写
        """
        # 默认实现：副号递增
    ...
    def calculate_basic_quantities(self, total_pieces: int, pieces_per_unit: int) -> Dict[str, int]:
        """
        计算基础数量
    ...
    def calculate_multi_level_quantities(self, total_pieces: int, pieces_per_box: int, 
                                       boxes_per_small_box: int, small_boxes_per_large_box: int = None) -> Dict[str, int]:
        """
        计算多级包装数量
    ...
    def calculate_overweight_distribution(self, items_per_set: int, sets_per_container: int, 
                                        item_in_container: int) -> int:
        """
        计算超重模式下的分配
        使用均分+余数分配策略
